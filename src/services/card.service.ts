...
export async function updateCard(cardId: string, input: UpdateCardInput, userId: string) {
  ...
  if (isColumnChange && input.columnId) {
    ...
    if (destColumn && destColumn.transitionMode !== "OFF") {
      ...
      if (ihlaller.length > 0) {
        if (destColumn.transitionMode === "ENFORCE" && !(input.force && role === "ADMIN")) {
          throw new ValidationError(
            `"${newColumnName}" sütununa geçiş için: ${ihlaller.join(", ")}`,
          );
        }
        transitionWarnings = ihlaller;
      }
    }
  }
  ...
  if (input.estimateMinutes !== undefined) updateData.estimateMinutes = input.estimateMinutes;
  if (input.assigneeIds !== undefined) {
  ...
  // transitionWarnings kalici bir alan degil - WARN modunda ihlal varsa bu
  // TEK cevaba ekleniyor, kartin kendisine yazilmiyor (route bunu okuyup
  // istemciye toast icin iletir).
  return transitionWarnings && transitionWarnings.length > 0
    ? { ...updated, transitionWarnings }
    : updated;
}
...
export async function deleteCard(cardId: string, userId: string) {
  ...
  // Otomatik "oksuz etiket" temizligi kaldirildi: Label proje seviyesinde
  // bilincli olusturulan bir katalog kaydi, kullaniminin sifira dusmesi
  // tanimin silinmesi anlamina gelmez. Tek kartin silinmesi projenin etiket
  // listesinden bir tanimi dusuruyor ve o labelId'yi tutan kayitli
  // gorunum/otomasyon kurallarini kiriyordu. Silme yalnizca deleteLabel ile.
